...
**(A) Re-reduce reference (`ChromaticSixPackVineyard`, re-reduce from first
...
addition can touch `n` rows). ∎ *(The coupled-pack update is implemented in
`IncrementalChromaticSixPack` and the `O(local)` cost is measured below; the
locality lemma is the substantive content.)*

Contrast: a from-scratch 6-pack recompute is `O(n^3)` dense / `O(n·m_⊥)` sparse
per query; the re-reduce (A) is `O(n)` per transposition; the target (B) is
...
1. **Re-reduce (A):** column additions vs. crossing position `i` — linear in
   `n − i` (the suffix length) per pack, confirming the suffix cost.
2. **Incremental (B):** column additions per transposition — flat in `n`
   (amortised `O(1)`), with rare `O(n)` spikes (the worst-case column op).
...
update. Both are implemented and measured below.

## Measured (re-reduce reference A), `theory/measure_placeholder_cost.py`
...
- **`corr(suffix length, column additions) = 0.92`** — the re-reduce's work
